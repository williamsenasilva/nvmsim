...
DramPerfModelNVM::DramPerfModelNVM(core_id_t core_id, UInt32 cache_block_size) :
    ...
    printf("[NVMSIM] [DramPerfModelNVM::DramPerfModelNVM] ...\n");

    SubsecondTime dram_latency = SubsecondTime::FS() * static_cast<uint64_t>(TimeConverter<float>::NStoFS(
            Sim()->getCfg()->getFloat(
                    "perf_model/dram/latency"))); // Operate in fs for higher precision before converting to uint64_t/SubsecondTime
    SubsecondTime dram_latency_stddev = SubsecondTime::FS() * static_cast<uint64_t>(TimeConverter<float>::NStoFS(
            Sim()->getCfg()->getFloat("perf_model/dram/nvm/standard_deviation")));
    ...
    printf("[NVMSIM] [DramPerfModelNVM::DramPerfModelNVM] ... done\n");
}
...
SubsecondTime
DramPerfModelNVM::getAccessLatency(SubsecondTime pkt_time, UInt64 pkt_size, core_id_t requester, IntPtr address,
    ...
    printf("[NVMSIM] [DramPerfModelNVM::getAccessLatency] m_num_accesses: %ld\n", m_num_accesses);
