...
def get_cxx_inuse(sim_root, clear_ldlibpath = False):
  pin_sim = None
  path_to_search = '%s/lib/pin_sim.so, %s/lib/sniper' %(sim_root, sim_root)
  for binary in ['%s/lib/pin_sim.so' % sim_root, '%s/lib/sniper' % sim_root]:
    if os.path.isfile(binary):
      pin_sim = binary
      print '[NVMSIM][DEBUG][RUNSN] [run_sniper.py] found: %s in %s' % (binary, path_to_search)
    else:
      print '[NVMSIM][DEBUG][RUNSN] [run_sniper.py] not found: %s in %s' % (binary, path_to_search)
  if not pin_sim:
    return None
  try:
...
def get_cxx_override(sim_root, pin_home, arch):
  ...
  print '[NVMSIM][DEBUG][RUNSN] [run_sniper.py] cxx_versions %s' % (cxx_versions)
...
def setup_env(sim_root, pin_home, arch, standalone = False, xed_home = None):
  ...
  print '[NVMSIM][DEBUG][RUNSN] [run_sniper.py] ld_library_path %s' % (ld_library_path)
